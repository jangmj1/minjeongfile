   ...
   if(key==37){//왼쪽키
      ...
      userbox.style.backgroundSize=`110%`
   		...
   		if( m_left-u_left > 0 && m_left-u_left < 100){
			...
			if(mhp<=0){															// 첫번쨰 몬스터가 죽으면
				monbox.style.backgroundImage=`url("img/${몬스터[0].img}")`	
				mhp_box.style.width =`${몬스터[0].hp}px`
				
				let rand=parseInt(Math.random()*20+1 )
				몬스터[0].hp-=rand
				몬스터[0].hp<0 ? 몬스터[0].hp=0 : 몬스터[0].hp=몬스터[0].hp
				
				document.querySelector('.mhp').innerHTML='hp:'+몬스터[0].hp
		   ...
		   userbox.style.backgroundImage=`url("img/막기.png")`
      	   userbox.style.backgroundSize=`110%`
...
function 몬스터치명타(){ //몬스터가 받는 데미지
	let rand=parseInt(Math.random()*20+1 )
	mhp-=rand
	console.log('rand',rand)
}
function 유저치명타(){ //유저가 받는ㄷ ㅔ미지
...
function 몬스터출력(){
	...
	for(i=0;i<몬스터.length;i++){
		...
		if(몬스터[i].hp<=0){
		monbox.style.backgroundImage=`url("img/${몬스터[i+1].img}")`	
		monbox.style.backgroundSize=`${90}%`	
		mhp_box.style.width =`${몬스터[i+1].hp}px`
		
		let rand=parseInt(Math.random()*20+1 )
		몬스터[i+1].hp-=rand
		몬스터[i+1].hp<0 ? 몬스터[i+1].hp=0 : 몬스터[i+1].hp=몬스터[i+1].hp
		
		document.querySelector('.mhp').innerHTML='hp:'+몬스터[i+1].hp 
